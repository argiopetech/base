...
void loadChaboyer(char *path, int filterSet){
  ...
  for(z=0 ; z < N_CHAB_Z ; z++) {                                       // foreach Chaboyer metallicity/isochrone file 
    ...
    for(y=0 ; y < N_CHAB_Y ; y++) {                                     // foreach Chaboyer metallicity/isochrone file 
      ...
      if((pChaboyer = fopen(tempFile,"r")) == NULL) {                   // open file 
      ...
      while(fgets(line,240,pChaboyer) != NULL) {  // load each Z=# and Y=# Chaboyer model for all ages 
        if(line[1] == 'M'){
          fgets(line,240,pChaboyer);
          sscanf(line,"%*s %*f %*f %*f %*f %lf",&cFeH[z]);
          if(fabs(cFeH[z]) < 0.02) cFeH[z] = 0.0;
        }
        else if(line[1] == 'A'){
          a++;
          ...
          sscanf(line,"%d %lf %*f %*f %*f %lf %lf %lf %lf %lf %lf %lf %lf",&cIso[z][y][a].eeps[i],
                 &cIso[z][y][a].mass[i],&cIso[z][y][a].mag[i][0],&cIso[z][y][a].mag[i][1],&cIso[z][y][a].mag[i][2],
                 &cIso[z][y][a].mag[i][3],&cIso[z][y][a].mag[i][4],&cIso[z][y][a].mag[i][5],&cIso[z][y][a].mag[i][6],
                 &cIso[z][y][a].mag[i][7]);
...
double deriveChabAgbTip(double newFeH, double newY, double newLogAge){
  ...
  for(a=iAge;a<iAge+2;a++){
    for(y=iY;y<iY+2;y++){
      for(z=iFeH;z<iFeH+2;z++){
        if(cIso[z][y][a].eeps[0] > newimin) newimin=cIso[z][y][a].eeps[0];
        if(cIso[z][y][a].eeps[cIso[z][y][a].numEeps-1] < newimax) newimax = cIso[z][y][a].eeps[cIso[z][y][a].numEeps-1];
      }
    }
  }
  ...
  for(m=0;m<neweep;m++){
    ...
    for(filt=0;filt<N_CHAB_FILTS;filt++) if(useFilt[filt]) isochrone.mag[m][filt]=0.0;
...
static void initIso(struct cIsochrone *newIso){
  ...
  for(i=0;i<MAX_CHAB_ENTRIES;i++){
    newIso->eeps[i] = 0;
    newIso->mass[i] = 0.0;
    for(f=0;f<N_CHAB_FILTS;f++) newIso->mag[i][f] = 99.9;
  }
}
/*
static void outputIso(struct cIsochrone *iso, FILE *wPtr){
