...
void loadBergeron(char *path, int filterSet){
    ...
    for(l=0;l<BERG_N_DA_LOG_G;l++){
        for(t=0;t<BERG_N_DA_TEFF;t++){
            ...
            for(f=0;f<BERG_NFILTS;f++) fscanf(pBergeron,"%lf ",&bMag[t][l][f]);
            if(filterSet == SDSS)
                for(f=0;f<5;f++) fscanf(pBergeron,"%lf ",&bMag[t][l][f]);
            fgets(line, 1000, pBergeron);
    ...
    for(l=0;l<BERG_N_DB_LOG_G;l++){
        for(t=0;t<BERG_N_DB_TEFF;t++){
            ...
            for(f=0;f<BERG_NFILTS;f++) fscanf(pBergeron,"%lf ",&bHeMag[t][l][f]);
            if(filterSet == SDSS)
                for(f=0;f<5;f++) fscanf(pBergeron,"%lf ",&bHeMag[t][l][f]);
