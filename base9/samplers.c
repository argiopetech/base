...
void propFieldStar(struct star *inputStar)
{
  ...
  if(rand1 > (*inputStar).clustStarProposalDens) (*inputStar).isFieldStar = 1;
  else (*inputStar).isFieldStar = 0;
...
double sampleT(double var, double nu)
{
   ...
   do {
      u = genrand_res53();
      //if(u < 1.e-15) u = 1.e-15;		/* (TvH) - trap for u = 0? shouldn't be necessary */
      u = 8.0 * sqrt(var) * (2.0 * u - 1.0);
      y = peak * genrand_res53();
      if(y < 1.e-15) y = 1.e-15;		/* (TvH) - trap for u = 0; does 53 bit resolution mean 1/(2^53)? */
      y = log(y);
...
S130: 
    ...
    if(p >= 1.0) goto S140; 
    sgamma = exp(log(p)/ a); 
    if(-log(genrand_res53()) < sgamma) goto S130; 
    return sgamma; 
S140: 
    sgamma = -log((b0-p)/ a); 
    if(-log(genrand_res53()) < (1.0-a)*log(sgamma)) goto S130; 
    return sgamma; 
}
