...
// Compute log prior density
{
  ...
  if(pStar->status[0] == BD) return 0.0;
...
//  mass2 = getMass2(pStar,pCluster);
  ...
  if(mass1 > 0.1 && mass1 <= pCluster->M_wd_up) {
    if((*pStar).isFieldStar) {
      ...
      if((*pStar).status[0] != WD) logPrior += logTDens((*pStar).massRatio, (*pStar).meanMassRatio, (*pStar).varMassRatio, DOF);
      return logPrior;
    }
    else {
      log_m1 = log10(mass1);
...
double logPriorClust(struct cluster *pCluster)
{
  ...
  else if (pCluster->parameter[IFMR_SLOPE] < 0.0) return -HUGE_VAL;
  if (pCluster->evoModels.IFMR == 11) {
    if (pCluster->parameter[IFMR_QUADCOEF] < 0.0) return -HUGE_VAL;
  }
  ...
  if (pCluster->evoModels.IFMR == 10) {
    ...
    if (slopeLower < 0.0 || slopeUpper < 0.0) return -HUGE_VAL;
  }
  ...
  if (pCluster->priorVar[YYY] > EPSILON) prior += (-0.5) * SQR(getParameter(pCluster,YYY) - pCluster->priorMean[YYY]) / pCluster->priorVar[YYY];
