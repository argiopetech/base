...
void calcPost(double *post, double dMass, double mag[][FILTS], double clusterAv, double *flux, 
  double *mass, struct cluster *pCluster, struct star *pStar);
...
double margEvolveWithBinary(struct cluster *pCluster, struct star *pStar)
{
  ...
  if(fabs(clusterAbs[0]) < EPS) calcAbsCoeffsForMarg(pCluster->evoModels.filterSet);
  ...
  double *mass, struct cluster *pCluster, struct star *pStar)
{
  ...
  while (filt < FILTS && isOverlap == 1) {
    if (useFilt[filt]) {
      diffLow = pow(10.0, ((pStar->obsPhot[obsFilt] - nSD * sqrt(pStar->variance[obsFilt])) / -2.5)) - 
                pow(10.0, (mag[0][filt] / -2.5)); 
      diffUp = pow(10.0, ((pStar->obsPhot[obsFilt] + nSD * sqrt(pStar->variance[obsFilt])) / -2.5)) - 
                pow(10.0, (mag[0][filt] / -2.5)); 
      if (diffLow <= 0.0 || diffUp <= 0.0 || diffLow == diffUp) {
        isOverlap = 0;
        
        /**** necessary here??? *****/
        for(j = 0; j < FILTS; j++) if(useFilt[j]) mag[2][j] = mag[0][j];
      }
      else {
