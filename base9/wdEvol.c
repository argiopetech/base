...

{
  ...
  if(cmpnt==0) mass = getMass1(pStar,pCluster);
  else mass         = getMass2(pStar,pCluster);
  ...
  if(thisPrecLogAge >= getParameter(pCluster,AGE)) {      // mcmc.c can cause this by adjusting masses and ages
    for(filt=0;filt < FILTS;filt++) if(useFilt[filt]) globalMags[filt] = -4.; // place at tip of RGB
  }
  else {
	  //Calculate logg
