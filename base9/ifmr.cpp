...
double linearIFMRshift(struct cluster *pCluster, double zamsMass);
double linearIFMR0(struct cluster *pCluster, double zamsMass);
double linearIFMRage(struct cluster *pCluster, double zamsMass);
double linearIFMRhighShift(struct cluster *pCluster, double zamsMass);
double linearIFMRedit(struct cluster *pCluster, double zamsMass);
...
double linearIFMRshift(struct cluster *pCluster, double zamsMass) 
{
  ...
  double wdMass = pCluster->parameter[IFMR_INTERCEPT] + 
    pCluster->parameter[IFMR_SLOPE] * (zamsMass - shiftMass);
...
{
  double wdMass = pCluster->parameter[IFMR_INTERCEPT] + 
    pCluster->parameter[IFMR_SLOPE] * (zamsMass - 27.0 + 2.6 * pCluster->parameter[AGE]);
  return wdMass;
}
...
{
  double wdMass = pCluster->parameter[IFMR_INTERCEPT] + 
    pCluster->parameter[IFMR_SLOPE] * zamsMass;
  return wdMass;
}
...
double quadraticIFMRshift(struct cluster *pCluster, double zamsMass) 
{
  ...
  double wdMass = pCluster->parameter[IFMR_INTERCEPT] + 
    pCluster->parameter[IFMR_SLOPE] * (zamsMass - shiftMass) +
    pCluster->parameter[IFMR_QUADCOEF] * (zamsMass - shiftMass) * (zamsMass - shiftMass);
...
double quadraticIFMRrotate(struct cluster *pCluster, double zamsMass) 
{
    ...
    double wdMass = pCluster->parameter[IFMR_INTERCEPT] - 0.5 * A +
        sqrt(0.25 * A * A - cLo * cUp - 1.0 / (sin(angle) * pCluster->parameter[IFMR_QUADCOEF]) * (zamsMass - shiftMass));
...
double piecewiseLinearIFMR(struct cluster *pCluster, double zamsMass)
{
  ...
  if (zamsMass < breakpointMass) {
    wdMass = pCluster->parameter[IFMR_INTERCEPT] + 
      pCluster->parameter[IFMR_SLOPE] * (zamsMass - shiftMass);
  }
  else {
    wdMass = pCluster->parameter[IFMR_INTERCEPT] + 
      pCluster->parameter[IFMR_SLOPE] * (breakpointMass - shiftMass) +
      pCluster->parameter[IFMR_QUADCOEF] * (zamsMass - breakpointMass);
  }
