...
int main(int argc, char *argv[])
{
  int     i, j,
          filt,
          numtasks,                   /* total number of MPI process in partitiion */
...
//  clustPar cp[N_GRID];
  ...
  if (taskid != MASTER) init_genrand(seed);
  ...
  for (m = index; m < index + chunksize; m++) {
    ...
    for (j = 0; j < mc.clust.nStars; j++) {
      ...
      if (mc.stars[j].status[0] == WD) {
        ...
        for (mass1 = 0.15; mass1 < mc.clust.M_wd_up; mass1 += dMass1) {
          if (wdLogPost[im] > maxWDLogPost) maxWDLogPost = wdLogPost[im];
          im++;
        }
        ...
        clusMemPost[m * nWDs + iWD] = mc.stars[j].clustStarPriorDens * postClusterStar /
          (mc.stars[j].clustStarPriorDens * postClusterStar + (1.0 - mc.stars[j].clustStarPriorDens) * fsLike);
...
static void readCmdData(struct chain *mc, struct ifmrGridControl *ctrl) 
{
    ...
    while (moreStars)
    {
        ...
        for(i = 0; i < ctrl->numFilts; i++)
        {
            ...
            if(mc->stars[j].obsPhot[i] < ctrl->filterPriorMin[i]) ctrl->filterPriorMin[i] = mc->stars[j].obsPhot[i];
            if(mc->stars[j].obsPhot[i] > ctrl->filterPriorMax[i]) ctrl->filterPriorMax[i] = mc->stars[j].obsPhot[i];
        }
        ...
        fscanf(ctrl->rData,"%lf %lf %d %lf %d",
               &(mc->stars[j].U), &(mc->stars[j].massRatio),&(mc->stars[j].status[0]),
               &(mc->stars[j].clustStarPriorDens),&(mc->stars[j].useDuringBurnIn));
...
static void initChain(struct chain *mc, const struct ifmrGridControl *ctrl) {
  ...
  for (j = 0; j < mc->clust.nStars; j++) {
    ...
    mc->stars[j].varU                  = 0.0;
    for(i = 0 ; i < 2 ; i++) mc->stars[j].wdType[i] = DA;
