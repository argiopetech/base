...
static void initIfmrMcmcControl(struct chain *mc, struct ifmrMcmcControl *ctrl) {
    ...
    if (mc->clust.evoModels.IFMR <= 10) ctrl->priorMean[IFMR_QUADCOEF] = 0.0001;
    else ctrl->priorMean[IFMR_QUADCOEF] = 0.08;
...
static void readCmdData(struct chain *mc, struct ifmrMcmcControl *ctrl) {
    ...
    for(i = 0; i < FILTS; i++){
        if(ctrl->useFilt[i]){
            ctrl->numFilts++;
            if (aFilt < 0) aFilt = i;                         // Sets this to a band we know we are using (for evolve)
        }
    }
...
//            printf("%g %g after, in %d\n", ctrl->filterPriorMax[i], ctrl->filterPriorMin[i], taskid);
        ...
        if(mc->stars[j].status[0]==3 ||
           (mc->stars[j].obsPhot[ctrl->iMag] >= ctrl->minMag && mc->stars[j].obsPhot[ctrl->iMag] <= ctrl->maxMag))
        {
            j++;
        }
    }
...
static void initChain(struct chain *mc, const struct ifmrMcmcControl *ctrl) {
    ...
    for (j = 0; j < mc->clust.nStars; j++) {
        ...
        mc->stars[j].varU                  = 0.0;
        for(i = 0 ; i < 2 ; i++) mc->stars[j].wdType[i] = 0;
...
static void propClustCorrelated(struct cluster *clust, const struct ifmrMcmcControl *ctrl) {
    ...
    for (p = 0; p < NPARAMS; p++) {
        if (ctrl->priorVar[p] > EPSILON) {
            for (k = 0; k <= p; k++) { /* propMatrix is lower diagonal */
