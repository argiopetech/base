...

{
   ...
   for(filt=8; filt<FILTS; filt++) useFilt[filt] = 1;   // but not the other crap
   ...
   for(filt=0;filt<FILTS;filt++) if(useFilt[filt]) fprintf(w_ptr,"%s1 ",getFilterName(filt));
   fprintf(w_ptr, "stage1 wdM1 wdType1 wdLogTeff1 ltau1 mass2 ");
   for(filt=0;filt<FILTS;filt++) if(useFilt[filt]) fprintf(w_ptr,"%s2 ",getFilterName(filt));
   fprintf(w_ptr, "stage2 wdM2 wdType2 wdLogTeff2 ltau2 ");
   for(filt=0;filt<FILTS;filt++) if(useFilt[filt]) fprintf(w_ptr,"%s " ,getFilterName(filt));
   fprintf(w_ptr, "\n");
   ...
   for(i=0;i < theCluster.nStars;i++) {                               // for all systems in the cluster
     do{
     ...
     for(filt=0;filt < FILTS;filt++){
       if(useFilt[filt]) fprintf(w_ptr, "%6.3f ", theStar.photometry[filt] < 99. ? theStar.photometry[filt] : 99.999);
     }
     fprintf(w_ptr,"%d %5.3f %d %5.3f %5.3f ", theStar.status[0],
             (theStar.status[0]==3 ? theStar.massNow[0] : 0.0), 0, theStar.wdLogTeff[0], ltau[0]);

     tempU = theStar.U;                                              // save so we can output the total photometry below
     if(theStar.status[0] != NSBH && theStar.status[0] != WD){
       if(genrand_res53() < fractionBinary) {                         // create binaries among appropriate fraction
         do{
     ...
     for (cmpnt=0;cmpnt < 2; cmpnt++) updateCount(&theStar, cmpnt);

     for(filt=0;filt < FILTS;filt++) if(useFilt[filt]) fprintf(w_ptr,"%6.3f ",theStar.photometry[filt] < 99. ? theStar.photometry[filt] : 99.999); // output photometry for whole system
     fprintf(w_ptr,"\n");

     // Update min and max
     if(theStar.photometry[2] < 97 && theStar.status[0]!=WD && theStar.status[1] != WD){
       if(theStar.photometry[2] < minV) minV = theStar.photometry[2];
       if(theStar.photometry[2] > maxV) maxV = theStar.photometry[2];
     }
   }
   ...
   for(i=0;i < nBrownDwarfs;i++) {                            // for all systems in the cluster
     ...
     for(filt=0;filt < FILTS;filt++){
       if(useFilt[filt]) fprintf(w_ptr,"%6.3f ",99.999);
     }
   ...
   printf("\n logClusAge     = %6.3f\n [Fe/H]         = %5.2f\n Y              = %5.2f",
          theCluster.parameter[AGE],theCluster.parameter[FEH],theCluster.parameter[YYY]);
   printf("\n modulus        = %5.2f\n Av             = %5.2f\n WDMassUp       = %4.1f",
          theCluster.parameter[MOD],theCluster.parameter[ABS],theCluster.M_wd_up);
   printf("\n fractionBinary = %5.2f\n",fractionBinary);

   printf("\n Totals:");
   printf("\n nSystems       = %d\n nStars         = %d\n nMSRG          = %d\n nWD            = %d",
          theCluster.nStars,nStars,nMSRG,nWD);
   printf("\n nNSBH          = %d\n massTotal      = %.2f\n MSRGMassTotal  = %.2f\n wdMassTotal    = %.2f\n",
...
//     setModels(&theCluster, DARTMOUTH); // Because the Yale models have trouble making field stars
   ...
   while(i < theCluster.nStars) {
     ...
     } while(theStar.photometry[2] < minV || theStar.photometry[2] > maxV
             || theStar.photometry[1]-theStar.photometry[2] < -0.5
             || theStar.photometry[1]-theStar.photometry[2] > 1.7);

     fprintf(w_ptr,"%4d %7.3f ",i+20001,getMass1(&theStar,&theCluster));
     for(filt=0;filt < FILTS;filt++) if(useFilt[filt]) fprintf(w_ptr, "%6.3f ", 99.999);
     fprintf(w_ptr,"%d %5.3f %d %5.3f %5.3f ", theStar.status[0],
             (theStar.status[0]==3 ? theStar.massNow[0] : 0.0), 0, theStar.wdLogTeff[0], ltau[0]);
     ...
     for(filt=0;filt < FILTS;filt++) if(useFilt[filt]) fprintf(w_ptr,"%9.6f ",theStar.photometry[filt]);
     fprintf(w_ptr,"\n");
